...
let realAudio = typeof Audio !== 'undefined' ? Audio : function () { return {} };// backup real audio
Audio = function (src) {
...
function randomFloor(x) {if ((x%1)<Math.random()) return Math.floor(x); return Math.ceil(x);}

function shuffle(array)
{
  let counter = array.length; var temp; var 
index;
...
let LoadScript = function (url, callback, error)
{
  ...
  return function (val)
	{
		...
		if (val>=1000000)
		{
			...
			if (base>=notations.length) {return 'Infinity';} notationValue=notations[base];
		}
...
let formatLong = [' thousand', ' million', ' billion', ' trillion', ' quadrillion', ' quintillion', ' sextillion', ' septillion', ' octillion', ' nonillion'];
...
let formatShort = ['k', 'M', 'B', 'T', 'Qa', 'Qi', 'Sx', 'Sp', 'Oc', 'No'];
...
let Beautify = function (val, floats)
{
  let negative = (val < 0);
  let decimal = '';
  ...
  let output = (val.toString().indexOf('e+') != -1 && format == 2) ? val.toPrecision(3).toString() : formatter(val).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
  // var output=formatter(val).toString().replace(/\B(?=(\d{3})+(?!\d))/g,',');
  if (output == '0') negative = false;
  return negative ? `-${output}` : output + decimal;
};
let shortenNumber = function (val)
{
  ...
  if (val >= 1000000 && isFinite(val))
  {
    ...
    if (ind == -1) return val;
    let str = '';
    for (let i = 0; i < ind; i++) { str += (i < 6 ? num[i] : '0'); }
    str += 'e+';
    str += num.split('e+')[1];
...
let SimpleBeautify = function (val)
{
  let str = val.toString();
  let str2 = '';
  for (let i in str)// add commas
  {
...
let beautifyInTextFilter = /(([\d]+[,]*)+)/g;// new regex
...
{
  let func = function (what) { what.ddesc = BeautifyInText(what.ddesc); };
...
*/
let locBlink = false;
let localizationNotFound = [];
let loc = function (id, params, baseline)
{
  let fallback = false;
...
let ModLanguage = function (id, json) {
  if (id == '*') id = locId;
  if (id != locId || !Langs[id]) return false;
  if (json['REPLACE ALL'])
  {
    let rep = function (str, from, to)
    {
    ...
    for (let i in json['REPLACE ALL'])
    {
      let to = json['REPLACE ALL'][i];
      for (let ii in locStrings)
      {
        if (Array.isArray(locStrings[ii]))
        {
          for (let iii in locStrings[ii])
          {
...
let getUpgradeName = function(name)
{
	...
	if (found) return loc(found); return name;
};
...
function b64_to_utf8(str) {
  try { return decodeURIComponent(Array.prototype.map.call(atob(str), (c) => {
...
function pack2(/* string */ values) {
  let chunks = values.match(/.{1,14}/g); var 
packed = '';
...
CanvasRenderingContext2D.prototype.fillPattern = function (img, X, Y, W, H, iW, iH, offX, offY)
{
  ...
  if (img.alt != 'blank')
  {
    var offX = offX || 0;
    var offY = offY || 0;
    if (offX < 0) { offX-=Math.floor(offX/iW)*iW; } if (offX > 0) { offX = (offX % iW) - iW; }
    if (offY < 0) { offY-=Math.floor(offY/iH)*iH; } if (offY > 0) { offY = (offY % iH) - iH; }
    for (let y = offY; y < H; y += iH) { for (let x = offX; x < W; x += iW) { this.drawImage(img, X + x, Y + y, iW, iH); } }
  }
};
...
function AddEvent(el,ev,func)
{
	...
	if (el.attachEvent) {var func2=function(){func.call(el);};el.attachEvent('on'+ev,func2);return [el,ev,func2];}
	return false;
}
...
let Pic = function(what)
{
	...
	if (Game.Loader.assetsLoading.indexOf(what)==-1) Game.Loader.Load([what]);
...
let OldPlaySound = function (url, vol)
{
  ...
  if (!Sounds[url]) { Sounds[url] = new Audio(url); Sounds[url].onloadeddata = function (e) { e.target.volume = (volume*Game.volume/100)**2; } }
  else if (Sounds[url].readyState >= 2) { Sounds[url].currentTime = 0; Sounds[url].volume = (volume*Game.volume/100)**2; }
  Sounds[url].play();
};
let SoundInsts = [];
let SoundI = 0;
...
var PlaySound = function (url, vol, pitchVar)
{
  ...
  else if (Sounds[url].readyState >= 2 && SoundInsts[SoundI].paused)
  {
    ...
    // sound.currentTime=0;
    sound.volume = (volume*volumeSetting/100)**2;
    if (pitchSupport)
    {
...
(function () {
  ...
  for (let i = 0; i < Game.modHooksNames.length; i++) { Game.modHooks[Game.modHooksNames[i]] = []; }
  ...
  Game.registerHook = function (hook, func)
  {
    ...
    {
      for (let i = 0; i < func.length; i++) { Game.registerHook(hook, func[i]); }
      return;
    }
  ...
  Game.removeHook = function (hook, func)
  {
    if (func.constructor === Array)
    {
      for (let i = 0; i < func.length; i++) { Game.removeHook(hook, func[i]); }
      return;
    }
    if (typeof func !== 'function') return;
...
Game.Launch = function ()
{
  ...
  let preloadImages = ['img/goldCookie.png', 'img/wrathCookie.png', 'img/spookyCookie.png', 'img/hearts.png', 'img/contract.png', 'img/wrathContract.png', 'img/bunnies.png', 'img/frostedReindeer.png'];
  ...
  Game.visible = true;
  AddEvent(document, 'visibilitychange', (e)=>  { if (document.visibilityState === 'hidden') Game.visible = false; else Game.visible = true; });
  ...
  else
  {
    ...
    let easterDay = (function(Y){var C = Math.floor(Y/100);var N = Y - 19*Math.floor(Y/19);var K = Math.floor((C - 17)/25);var I = C - Math.floor(C/4) - Math.floor((C - K)/3) + 19*N + 15;I = I - 30*Math.floor((I/30));I = I - Math.floor(I/28)*(1 - Math.floor(I/28)*Math.floor(29/(I + 1))*Math.floor((21 - N)/11));var J = Y + Math.floor(Y/4) + I + 2 - C + Math.floor(C/4);J = J - 7*Math.floor(J/7);var L = I - J;var M = 3 + Math.floor((L + 40)/44);var D = L + 28 - 31*Math.floor(M/4);return new Date(Y,M-1,D);}(year));
  ...
  Game.Init = function ()
  {
    ...
    Game.resize = function ()
    {
      ...
      let prevW = Game.windowW;
      let prevH = Game.windowH;
    ...
    Game.getTooltip = function(text,origin,isCrate)
		{
			...
			return 'onMouseOut="Game.tooltip.shouldHide=1;" onMouseOver="Game.tooltip.dynamic=0;Game.tooltip.draw(this,\''+escape(text)+'\',\''+origin+'\');Game.tooltip.wobble();"';
		};
    ...
    Game.GrabDataResponse = function (response)
    {
      ...
      try {
        ...
        if (typeof r.grandma !== 'undefined' && r.grandma != '')
        {
          ...
          Game.customGrandmaNames = Game.customGrandmaNames.filter((el)=>  { return el != ''; });
        }
    ...
    if (!App)
    {
      ...
      AddEvent(l('changeLanguage'), 'click', ()
			=> {
    ...
    {
      return (cookies/1000000000000)**(1/Game.HCfactor);
    };
    Game.HowManyCookiesReset = function (chips)// how many cookies [chips] are worth
    {
      ...
      return chips**Game.HCfactor * 1000000000000;
    };
    Game.gainedPrestige = 0;
    ...
    {
      let {icon} = Game.ascensionModes[Game.nextAscensionMode];
    ...
    Game.PickAscensionMode = function ()
    {
      ...
      {
        let {icon} = Game.ascensionModes[i];
      ...
      Game.Prompt(
`<id PickChallengeMode><h3>${loc("Select a challenge mode")}</h3>`+
...
'widePrompt'
    ...
    Game.attachTooltip(l('ascendData1'), ()=>  { return `<div style="min-width:300px;text-align:center;font-size:11px;padding:8px;" id="tooltipAscendData1">(<b>${Beautify(Game.heavenlyChips)}</b>)<div class="line"></div>${loc("Each prestige level grants you a permanent <b>+%1% CpS</b>.<br>The more levels you have, the more cookies they require.",1)}</div>`; }, 'bottom-right');
    Game.attachTooltip(l('ascendData2'), ()=>  { return `<div style="min-width:300px;text-align:center;font-size:11px;padding:8px;" id="tooltipAscendData2">(<b>${loc("%1 heavenly chip",LBeautify(Game.heavenlyChips))}</b>)<div class="line"></div>${loc("Heavenly chips are used to buy heavenly upgrades.<br>You gain <b>1 chip</b> every time you gain a prestige level.")}</div>`; }, 'bottom-right');

    Game.UpdateAscensionModePrompt();

    AddEvent(l('ascendButton'), 'click', ()=>  {
      PlaySound('snd/tick.mp3');
      Game.Reincarnate();
    });
      ...
      if (Game.mouseDown && !Game.promptOn)
      {
        ...
        if (Game.DebuggingPrestige)
        {
          if (Game.SelectedHeavenlyUpgrade)
          {
            ...
            let {posX} = me;// Math.round(me.posX/Game.AscendGridSnap)*Game.AscendGridSnap;
            let {posY} = me;// Math.round(me.posY/Game.AscendGridSnap)*Game.AscendGridSnap;
    ...
    Game.PurchaseHeavenlyUpgrade = function (what)
    {
      ...
      {
        if (l(`heavenlyUpgrade${what}`)) { let rect = l(`heavenlyUpgrade${what}`).getBounds(); Game.SparkleAt((rect.left + rect.right) / 2, (rect.top + rect.bottom) / 2 - 24); }
        // Game.BuildAscendTree();
      }
    };
    Game.BuildAscendTree = function (justBought)
    {
      ...
      {
        var me = Game.PrestigeUpgrades[i];
        let prevCanBePurchased = me.canBePurchased;
        me.canBePurchased = 1;
      ...
      {
        setTimeout(()=>  { PlaySound(`snd/pop${Math.floor(Math.random()*3+1)}.mp3`, 0.5); }, (0.2 + i * 0.1) * 1000);
      }
      ...
      setTimeout(()=>  { Game.tooltip.shouldHide = true; }, 100);
    };
    ...
    {
      let str = `<div style="padding:8px;width:400px;font-size:11px;text-align:center;" id="tooltipLumps">${
    ...
    Game.harvestLumps = function (amount, silent)
    {
      ...
      if (!silent)
      {
        let rect = l('lumpsIcon2').getBounds(); Game.SparkleAt((rect.left + rect.right) / 2, (rect.top + rect.bottom) / 2 - 24 + 32 - TopBarOffset);
    ...
    Game.ClickCookie = function (e, amount)
    {
      ...
      else
      {
        ...
        var amount = amount || Game.computedMouseCps;
        Game.Earn(amount);
    ...
    if (!Game.touchEvents)
    {
      ...
      AddEvent(bigCookie, 'mousedown', (event)=>  { Game.BigCookieState = 1; if (Game.prefs.cookiesound) { Game.playCookieClickSound(); } if (event) event.preventDefault(); });
      AddEvent(bigCookie, 'mouseup', (event)=>  { Game.BigCookieState = 2; if (event) event.preventDefault(); });
      AddEvent(bigCookie, 'mouseout', (event)=>  { Game.BigCookieState = 0; });
      AddEvent(bigCookie, 'mouseover', (event)=>  { Game.BigCookieState = 2; });
      AddEvent(document, 'mousemove', Game.GetMouseCoords);
      AddEvent(document, 'mousedown', (event)=>  { Game.lastActivity = Game.time; Game.mouseDown = 1; Game.clickFrom = event.target; });
      AddEvent(document, 'mouseup', (event)=>  { Game.lastActivity = Game.time; Game.mouseDown = 0; Game.clickFrom = 0; });
      AddEvent(document, 'click', (event)=>  { Game.lastActivity = Game.time; Game.Click = 1; Game.lastClickedEl = event.target; Game.clickFrom = 0; });
    ...
    else
    {
      ...
      AddEvent(bigCookie, 'touchstart', (event)=>  { Game.BigCookieState = 1; if (event) event.preventDefault(); });
      AddEvent(bigCookie, 'touchend', (event)=>  { Game.BigCookieState = 0; if (event) event.preventDefault(); });
      // AddEvent(document,'touchmove',Game.GetMouseCoords);
      AddEvent(document, 'mousemove', Game.GetMouseCoords);
      AddEvent(document, 'touchstart', (event)=>  { Game.lastActivity = Game.time; Game.mouseDown = 1; });
      AddEvent(document, 'touchend', (event)=>  { Game.lastActivity = Game.time; Game.mouseDown = 0; });
      AddEvent(document, 'touchend', (event)=>  { Game.lastActivity = Game.time; Game.Click = 1; });
    }
    ...
    Game.effs = {};
    Game.eff = function(name,def){if (typeof Game.effs[name]==='undefined') return (typeof def==='undefined'?1:def); return Game.effs[name];};
    ...
    Game.CalculateGains = function ()
    {
      ...
      let {n} = Game.shimmerTypes['golden'];
    ...
    Game.updateShimmers = function ()// run shimmer functions, kill overtimed shimmers and spawn new ones
    {
      ...
      for (var i in Game.shimmerTypes)
      {
        ...
        if (me.spawnsOnTimer && me.spawnConditions())// only run on shimmer types that work on a timer
        {
          ...
          {
            me.time++;
            if (Math.random() < Math.max(0,(me.time-me.minTime)/(me.maxTime-me.minTime))**5)
            {
    ...
    AddEvent(Game.darkenL, 'click', ()=>  { if (Game.promptNoClose) {} else { Game.Click = 0; PlaySound('snd/tickOff.mp3'); Game.ClosePrompt(); } });
    ...
    Game.Prompt = function (content, options, updateFunc, style)
    {
      ...
      if (str.indexOf('<id ') == 0)
      {
        let id = str.substring(4, str.indexOf('>'));
    ...
    Game.removeClass = function (what) { let i = Game.cssClasses.indexOf(what); if (i != -1) { Game.cssClasses.splice(i, 1); }Game.updateClasses(); };
    ...
    Game.showLangSelection = function (firstLaunch)
    {
      ...
      Game.Prompt(
`<id ChangeLanguage>${firstLaunch?'<noClose>':''}<h3 id="languageSelectHeader">${loc("Change language")}</h3>`+
				`<div class="line"></div>${
    ...
    Game.UpdateMenu = function ()
    {
      ...
      else if (Game.onMenu == 'stats')
      {
        ...
        let {icon} = Game.ascensionModes[Game.ascensionMode];
      ...
      if (App)
      {
        let anchors = l('menu').getElementsByTagName('a');
    ...
    AddEvent(l('prefsButton'), 'click', ()=>  { Game.ShowMenu('prefs'); });
    AddEvent(l('statsButton'), 'click', ()=>  { Game.ShowMenu('stats'); });
    AddEvent(l('logButton'), 'click', ()=>  { Game.ShowMenu('log'); });
    AddEvent(l('legacyButton'), 'click', ()=>  { PlaySound('snd/tick.mp3'); Game.Ascend(); });
    ...
    Game.getNewTicker = function (manual)// note : "manual" is true if the ticker was clicked, but may also be true on startup etc
    {
      ...
      {
        let animals = ['newts', 'penguins', 'scorpions', 'axolotls', 'puffins', 'porpoises', 'blowfish', 'horses', 'crayfish', 'slugs', 'humpback whales', 'nurse sharks', 'giant squids', 'polar bears', 'fruit bats', 'frogs', 'sea squirts', 'velvet worms', 'mole rats', 'paramecia', 'nematodes', 'tardigrades', 'giraffes', 'monkfish', 'wolfmen', 'goblins', 'hippies'];
    ...
    Game.Object = function (name, commonName, desc, icon, iconColumn, art, price, cps, buyFunction)
    {
      ...
      if (this.n != 0)
      {
        ...
        this.baseCps = Math.ceil(((this.n*1)**(this.n*0.5+2)) * 10) / 10;// 0.45 used to be 0.5
        ...
        var digits = 10**Math.ceil(Math.log(Math.ceil(this.baseCps))/Math.LN10) / 100;
        this.baseCps = Math.round(this.baseCps / digits) * digits;

        this.basePrice = (this.n * 1 + 9 + (this.n < 5 ? 0 : (this.n-5)**1.75 * 5)) * 10**this.n * (Math.max(1, this.n - 14));
        // this.basePrice=(this.n*2.5+7.5)*Math.pow(10,this.n);
        var digits = 10**Math.ceil(Math.log(Math.ceil(this.basePrice))/Math.LN10) / 100;
      ...
      {
        let price = this.basePrice * Game.priceIncrease**Math.max(0,this.amount-this.free);
      ...
      this.getSumPrice = function (amount)// return how much it would cost to buy [amount] more of this building
      {
        ...
        {
          price += this.basePrice * Game.priceIncrease**Math.max(0,i-this.free);
        }
        price = Game.modifyBuildingPrice(this, price);
        return Math.ceil(price);
      };
      ...
      this.sell = function (amount, bypass)
      {
        ...
        if (success && Game.shimmerTypes.golden.n <= 0 && Game.auraMult('Dragon Orbs') > 0)
        {
          ...
          {
            let buffsN = 0;
            for (let ii in Game.buffs) { buffsN++; }
            if (buffsN == 0)
            {
      ...
      this.tooltip = function ()
      {
        ...
        if (Game.prefs.screenreader)
        {
          ...
          let ariaLabel = l(`ariaReader-product-${me.id}`);
          if (ariaLabel) ariaLabel.innerHTML = ariaText.replace(/(<([^>]+)>)/gi, ' ');
        }
        ...
        this.draw = function ()
        {
          ...
          if (i != iT)
          {
            ...
            // if (i<iT)
            {
              ...
              if (frames > 1) { frame = prevFrame + Math.floor(Math.random() * (frames - 1) + 1); frame%=frames; }
              prevFrame = frame;
              this.pics.push({ x: Math.floor(x), y: Math.floor(y), z: y, pic: usedPic, id: i, frame });
              i++;
              added++;
            }
            while (i > iT)
    ...
    // define objects
    new Game.Object('Cursor', 'cursor|cursors|clicked|[X] extra finger|[X] extra fingers', 'Autoclicks once every 10 seconds.', 0, 0, {}, 15, ((me)=> {
			var add=0;
			if (Game.Has('Thousand fingers')) add+=		0.1;
			if (Game.Has('Million fingers')) add*=		5;
			if (Game.Has('Billion fingers')) add*=		10;
    ...
    new Game.Object('Grandma', 'grandma|grandmas|baked|Grandmas are [X] year older|Grandmas are [X] years older', 'A nice grandma to bake more cookies.', 1, 1, { pic(i){
			var list=['grandma'];
		...
		}, bg: 'grandmaBackground.png', xV: 8, yV: 8, w: 32, rows: 3, x: 0, y: 16 }, 100, ((me)=> {
			var mult=1;
			for (var i in Game.GrandmaSynergies)
			{
    ...
    Game.last.iconFunc = function (type) {
      let grandmaIcons = [[0, 1], [0, 2], [1, 2], [2, 2]];
    ...
    new Game.Object('Farm', 'farm|farms|harvested|[X] more acre|[X] more acres', 'Grows cookie plants from cookie seeds.', 3, 2, { base: 'farm', xV: 8, yV: 8, w: 64, rows: 2, x: 0, y: 16 }, 500, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
    ...
    new Game.Object('Mine', 'mine|mines|mined|[X] mile deeper|[X] miles deeper', 'Mines out cookie dough and chocolate chips.', 4, 3, { base: 'mine', xV: 16, yV: 16, w: 64, rows: 2, x: 0, y: 24 }, 10000, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Factory', 'factory|factories|mass-produced|[X] additional patent|[X] additional patents', 'Produces large quantities of cookies.', 5, 4, { base: 'factory', xV: 8, yV: 0, w: 64, rows: 1, x: 0, y: -22 }, 3000, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Bank', 'bank|banks|banked|Interest rates [X]% better|Interest rates [X]% better', 'Generates cookies from interest.', 6, 15, { base: 'bank', xV: 8, yV: 4, w: 56, rows: 1, x: 0, y: 13 }, 0, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Temple', 'temple|temples|discovered|[X] sacred artifact retrieved|[X] sacred artifacts retrieved', 'Full of precious, ancient chocolate.', 7, 16, { base: 'temple', xV: 8, yV: 4, w: 72, rows: 2, x: 0, y: -5 }, 0, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Wizard tower', 'wizard tower|wizard towers|summoned|Incantations have [X] more syllable|Incantations have [X] more syllables', 'Summons cookies with magic spells.', 8, 17, { base: 'wizardtower', xV: 16, yV: 16, w: 48, rows: 2, x: 0, y: 20 }, 0, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Shipment', 'shipment|shipments|shipped|[X] galaxy fully explored|[X] galaxies fully explored', 'Brings in fresh cookies from the cookie planet.', 9, 5, { base: 'shipment', xV: 16, yV: 16, w: 64, rows: 1, x: 0, y: 0 }, 40000, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Alchemy lab', 'alchemy lab|alchemy labs|transmuted|[X] primordial element mastered|[X] primordial elements mastered', 'Turns gold into cookies!', 10, 6, { base: 'alchemylab', xV: 16, yV: 16, w: 64, rows: 2, x: 0, y: 16 }, 200000, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Portal', 'portal|portals|retrieved|[X] dimension enslaved|[X] dimensions enslaved', 'Opens a door to the Cookieverse.', 11, 7, { base: 'portal', xV: 32, yV: 32, w: 64, rows: 2, x: 0, y: 0 }, 1666666, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Time machine', 'time machine|time machines|recovered|[X] century secured|[X] centuries secured', 'Brings cookies from the past, before they were even eaten.', 12, 8, { base: 'timemachine', xV: 32, yV: 32, w: 64, rows: 1, x: 0, y: 0 }, 123456789, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Antimatter condenser', 'antimatter condenser|antimatter condensers|condensed|[X] extra quark flavor|[X] extra quark flavors', 'Condenses the antimatter in the universe into cookies.', 13, 13, { base: 'antimattercondenser', xV: 0, yV: 64, w: 64, rows: 1, x: 0, y: 0 }, 3999999999, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Prism', 'prism|prisms|converted|[X] new color discovered|[X] new colors discovered', 'Converts light itself into cookies.', 14, 14, { base: 'prism', xV: 16, yV: 4, w: 64, rows: 1, x: 0, y: 20 }, 75000000000, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Chancemaker', 'chancemaker|chancemakers|spontaneously generated|Chancemakers are powered by [X]-leaf clovers|Chancemakers are powered by [X]-leaf clovers', 'Generates cookies out of thin air through sheer luck.', 15, 19, { base: 'chancemaker', xV: 8, yV: 64, w: 64, rows: 1, x: 0, y: 0, rows: 2 }, 77777777777, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Fractal engine', 'fractal engine|fractal engines|made from cookies|[X] iteration deep|[X] iterations deep', 'Turns cookies into even more cookies.', 16, 20, { base: 'fractalEngine', xV: 8, yV: 64, w: 64, rows: 1, x: 0, y: 0 }, 12345678987654321, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    new Game.Object('Javascript console', 'javascript console|javascript consoles|programmed|Equipped with [X] external library|Equipped with [X] external libraries', 'Creates cookies from the very code this game was written in.', 17, 32, { base: 'javascriptconsole', xV: 8, yV: 64, w: 14, rows: 1, x: 8, y: -32, frames: 2 }, 12345678987654321, ((me)=> {
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
    ...
    Game.Upgrade.prototype.getPrice = function ()
    {
      ...
      if (this.pool != 'prestige')
      {
        ...
        if (Game.Has('Five-finger discount')) price *= 0.99**(Game.Objects['Cursor'].amount/100);
    ...
    Game.Upgrade.prototype.canBuy = function()
		{
			...
			if (Game.cookies>=this.getPrice()) return true; return false;
		};
    ...
    Game.Upgrade.prototype.isVaulted = function()
		{
			if (Game.vault.indexOf(this.id)!=-1) return true; return false;
		};
    ...
    Game.CountsAsUpgradeOwned = function(pool)
		{
			if (pool=='' || pool=='cookie' || pool=='tech') return true; return false;
		};
    ...
    Game.Unlock = function (what)
    {
      ...
      else { for (let i in what) { Game.Unlock(what[i]); } }
    };
    Game.Lock = function (what)
    {
      ...
      else { for (let i in what) { Game.Lock(what[i]); } }
    };
    ...
    {
      let upgrade = new Game.Upgrade(obj.name, getStrCookieProductionMultiplierPlus(Beautify((typeof (obj.power) === 'function' ? obj.power(obj) : obj.power), 1)) + (EN ? `<q>${obj.desc}</q>` : ''), obj.price, obj.icon);
    ...
    Game.UnlockTiered = function (me)
    {
      ...
      for (var i in me.synergies) { let syn = me.synergies[i]; if (Game.Has(Game.Tiers[syn.tier].req) && syn.buildingTie1.amount >= Game.Tiers[syn.tier].unlock && syn.buildingTie2.amount >= Game.Tiers[syn.tier].unlock) Game.Unlock(syn.name); }
    };
    ...
    Game.GrandmaSynergy = function (name, desc, building)
    {
      ...
      let grandmaNumber = loc('%1 grandma', LBeautify(building.id - 1));
      ...
      let upgrade = new Game.Upgrade(name, desc, building.basePrice * Game.Tiers[2].price, [10, 9], (()=> {Game.Objects['Grandma'].redraw();}));
    ...
    new Game.Upgrade('Bingo center/Research facility', `${loc("Grandma-operated science lab and leisure club.<br>Grandmas are <b>4 times</b> as efficient.<br><b>Regularly unlocks new upgrades</b>.")}<q>What could possibly keep those grandmothers in check?...<br>Bingo.</q>`, 1000000000000000, [11, 9], (()=> {Game.SetResearch('Specialized chocolate chips');})); Game.last.noPerm = 1;

    order = 15000;
    new Game.Upgrade('Specialized chocolate chips', `${getStrCookieProductionMultiplierPlus(1)}<q>Computer-designed chocolate chips. Computer chips, if you will.</q>`, 1000000000000000, [0, 9], (()=> {Game.SetResearch('Designer cocoa beans');})); Game.last.pool = 'tech';
    new Game.Upgrade('Designer cocoa beans', `${getStrCookieProductionMultiplierPlus(2)}<q>Now more aerodynamic than ever!</q>`, 2000000000000000, [1, 9], (()=> {Game.SetResearch('Ritual rolling pins');})); Game.last.pool = 'tech';
    new Game.Upgrade('Ritual rolling pins', `${loc("%1 are <b>twice</b> as efficient.",cap(Game.Objects['Grandma'].plural))}<q>The result of years of scientific research!</q>`, 4000000000000000, [2, 9], (()=> {Game.SetResearch('Underworld ovens');})); Game.last.pool = 'tech';
    new Game.Upgrade('Underworld ovens', `${getStrCookieProductionMultiplierPlus(3)}<q>Powered by science, of course!</q>`, 8000000000000000, [3, 9], (()=> {Game.SetResearch('One mind');})); Game.last.pool = 'tech';
    new Game.Upgrade('One mind', `${loc("Each %1 gains <b>+%2 base CpS per %3</b>.",[loc("grandma"),'0.0<span></span>2',loc("grandma")])}<div class="warning">${loc("Note: the grandmothers are growing restless. Do not encourage them.")}</div><q>We are one. We are many.</q>`, 16000000000000000, [4, 9], (()=> {Game.elderWrath=1;Game.SetResearch('Exotic nuts');Game.storeToRefresh=1;})); Game.last.pool = 'tech';
    ...
    new Game.Upgrade('Exotic nuts', `${getStrCookieProductionMultiplierPlus(4)}<q>You'll go crazy over these!</q>`, 32000000000000000, [5, 9], (()=> {Game.SetResearch('Communal brainsweep');})); Game.last.pool = 'tech';
    new Game.Upgrade('Communal brainsweep', `${EN?'Each grandma gains another <b>+0.0<span></span>2 base CpS per grandma</b>.':loc("Each %1 gains <b>+%2 base CpS per %3</b>.",[loc("grandma"),'0.0<span></span>2',loc("grandma")])}<div class="warning">${loc("Note: proceeding any further in scientific research may have unexpected results. You have been warned.")}</div><q>We fuse. We merge. We grow.</q>`, 64000000000000000, [6, 9], (()=> {Game.elderWrath=2;Game.SetResearch('Arcane sugar');Game.storeToRefresh=1;})); Game.last.pool = 'tech';
    new Game.Upgrade('Arcane sugar', `${getStrCookieProductionMultiplierPlus(5)}<q>Tastes like insects, ligaments, and molasses.</q>`, 128000000000000000, [7, 9], (()=> {Game.SetResearch('Elder Pact');})); Game.last.pool = 'tech';
    new Game.Upgrade('Elder Pact', `${loc("Each %1 gains <b>+%2 base CpS per %3</b>.",[loc("grandma"),'0.0<span></span>5',loc("portal")])}<div class="warning">${loc("Note: this is a bad idea.")}</div><q>squirm crawl slither writhe<br>today we rise</q>`, 256000000000000000, [8, 9], (()=> {Game.elderWrath=3;Game.storeToRefresh=1;})); Game.last.pool = 'tech';
    new Game.Upgrade('Elder Pledge', `${loc("Contains the wrath of the elders, at least for a while.")}<q>This is a simple ritual involving anti-aging cream, cookie batter mixed in the moonlight, and a live chicken.</q>`, 1, [9, 9], (()
		=> {
			Game.elderWrath=0;
			Game.pledges++;
    ...
    Game.last.timerDisplay = function(){if (!Game.Upgrades['Elder Pledge'].bought) return -1; return 1-Game.pledgeT/Game.getPledgeDuration();};
    Game.last.priceFunc = function () { return 8**Math.min(Game.pledges+2,14); };
    ...
    order = 15000;
    new Game.Upgrade('Elder Covenant', `${loc("Puts a permanent end to the elders' wrath, at the cost of %1% of your CpS.",5)}<q>This is a complicated ritual involving silly, inconsequential trivialities such as cursed laxatives, century-old cacao, and an infant.<br>Don't question it.</q>`, 66666666666666, [8, 9], (()
		=> {
			Game.pledgeT=0;
    ...
    new Game.Upgrade('Revoke Elder Covenant', `${loc("You will get %1% of your CpS back, but the grandmatriarchs will return.",5)}<q>we<br>rise<br>again</q>`, 6666666666, [8, 9], (()
		=> {
    ...
    {
      let num = 0;
      for (let i in Game.halloweenDrops) { if (Game.Has(Game.halloweenDrops[i])) num++; }
      return num;
    };
    ...
    order = 25000;
    new Game.Upgrade('A festive hat', `<b>${loc("Unlocks... something.")}</b><q>Not a creature was stirring, not even a mouse.</q>`, 25, [19, 9], (()
		=> {
    ...
    new Game.Upgrade('Season savings', `${loc("All buildings are <b>%1% cheaper</b>.",1)}<br>${loc("Cost scales with Santa level.")}<q>By Santa's beard, what savings!<br>But who will save us?</q>`, 2525, [16, 9], (()=> {Game.storeToRefresh=1;}));
    new Game.Upgrade('Toy workshop', `${loc("All upgrades are <b>%1% cheaper</b>.",5)}<br>${loc("Cost scales with Santa level.")}<q>Watch yours-elf around elvesdroppers who might steal our production secrets.<br>Or elven worse!</q>`, 2525, [16, 9], (()=> {Game.upgradesToRebuild=1;}));
    ...
    order = 25000;
    new Game.Upgrade('Santa\'s dominion', `${getStrCookieProductionMultiplierPlus(20)}<br>${loc("All buildings are <b>%1% cheaper</b>.",1)}<br>${loc("All upgrades are <b>%1% cheaper</b>.",2)}<q>My name is Claus, king of kings;<br>Look on my toys, ye Mighty, and despair!</q>`, 2525252525252525, [19, 10], (()=> {Game.storeToRefresh=1;}));
    ...
    {
      let num = 0;
      for (let i in Game.heartDrops) { if (Game.Has(Game.heartDrops[i])) num++; }
      return num;
    };
    ...
    new Game.Upgrade('Season switcher', `${loc("Allows you to <b>trigger seasonal events</b> at will, for a price.")}<q>There will always be time.</q>`, 1111, [16, 6], (()=> {for (var i in Game.seasons){Game.Unlock(Game.seasons[i].trigger);}})); Game.last.pool = 'prestige'; Game.last.parents = ['Heralds'];
    ...
    order = 40000;
    new Game.Upgrade('Eternal seasons', `${loc("Seasons now last forever.")}<q>Season to taste.</q>`, 7, [16, 6], (()=> {for (var i in Game.seasons){Game.Unlock(Game.seasons[i].trigger);}}));// debug purposes only
    Game.last.pool = 'debug';
    ...
    new Game.Upgrade('Faberge egg', `${loc("All buildings and upgrades are <b>%1% cheaper</b>.",1)}<br>${loc("Cost scales with how many eggs you own.")}<q>This outrageous egg is definitely fab.</q>`, eggPrice2, [14, 12], (()=> {Game.storeToRefresh=1;}));
    ...
    new Game.Upgrade('Chocolate egg', `${loc("Contains <b>a lot of cookies</b>.")}<br>${loc("Cost scales with how many eggs you own.")}<q>Laid by the elusive cocoa bird. There's a surprise inside!</q>`, eggPrice2, [18, 12], (()
		=> {
    ...
    {
      let num = 0;
      for (let i in Game.easterEggs) { if (Game.Has(Game.easterEggs[i])) num++; }
      return num;
    };
    for (var i in Game.eggDrops)// scale egg prices to how many eggs you have
    { Game.Upgrades[Game.eggDrops[i]].priceFunc = function () { return 2**Game.GetHowManyEggs() * 999; } }

    for (var i in Game.rareEggDrops)
    { Game.Upgrades[Game.rareEggDrops[i]].priceFunc = function () { return 3**Game.GetHowManyEggs() * 999; } }
    ...
    Game.AssignPermanentSlot = function (slot)
    {
      ...
      for (var i in Game.Upgrades)
      {
        ...
        {
          let fail = 0;
          for (let ii in Game.permanentUpgrades) { if (Game.permanentUpgrades[ii] == me.id) fail = 1; }// check if not already in another permaslot
      ...
      Game.Prompt(
`<id PickPermaUpgrade><h3>${loc("Pick an upgrade to make permanent")}</h3>`+
      ...
      , [[loc('Confirm'), `Game.permanentUpgrades[${slot}]=Game.SelectingPermanentUpgrade;Game.BuildAscendTree();Game.ClosePrompt();`], loc('Cancel')],
0,
'widePrompt'
);
    };
    ...
    new Game.Upgrade('Angels', `${desc(10,15)}<q>Lowest-ranking at the first sphere of pastry heaven, angels are tasked with delivering new recipes to the mortals they deem worthy.</q>`, angelPriceFactor**1, [0, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Twin Gates of Transcendence'];
    new Game.Upgrade('Archangels', `${desc(10,25)}<q>Members of the first sphere of pastry heaven, archangels are responsible for the smooth functioning of the world's largest bakeries.</q>`, angelPriceFactor**2, [1, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Angels'];
    new Game.Upgrade('Virtues', `${desc(10,35)}<q>Found at the second sphere of pastry heaven, virtues make use of their heavenly strength to push and drag the stars of the cosmos.</q>`, angelPriceFactor**3, [2, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Archangels'];
    new Game.Upgrade('Dominions', `${desc(10,45)}<q>Ruling over the second sphere of pastry heaven, dominions hold a managerial position and are in charge of accounting and regulating schedules.</q>`, angelPriceFactor**4, [3, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Virtues'];
    new Game.Upgrade('Cherubim', `${desc(10,55)}<q>Sieging at the first sphere of pastry heaven, the four-faced cherubim serve as heavenly bouncers and bodyguards.</q>`, angelPriceFactor**5, [4, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Dominions'];
    new Game.Upgrade('Seraphim', `${desc(10,65)}<q>Leading the first sphere of pastry heaven, seraphim possess ultimate knowledge of everything pertaining to baking.</q>`, angelPriceFactor**6, [5, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Cherubim'];
    new Game.Upgrade('God', `${desc(10,75)}<q>Like Santa, but less fun.</q>`, angelPriceFactor**7, [6, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Seraphim'];
    ...
    new Game.Upgrade('Chimera', `${loc("Synergy upgrades are <b>%1% cheaper</b>.",2)}<br>${loc("You gain another <b>+%1%</b> of your regular CpS while the game is closed.",5)}<br>${loc("You retain optimal cookie production while the game is closed for <b>%1 more days</b>.",2)}<q>More than the sum of its parts.</q>`, angelPriceFactor**9, [24, 7]); Game.last.pool = 'prestige'; Game.last.parents = ['God', 'Lucifer', 'Synergies Vol. II'];
    ...
    var func = function () {
      if (Game.Has('Residual luck'))
      {
        let bonus = 0;
        let upgrades = Game.goldenCookieUpgrades;
        for (let i in upgrades) { if (Game.Has(upgrades[i])) bonus++; }
    ...
    new Game.Upgrade('Belphegor', `${desc(2)}<q>A demon of shortcuts and laziness, Belphegor commands machines to do work in his stead.</q>`, angelPriceFactor**1, [7, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Twin Gates of Transcendence'];
    new Game.Upgrade('Mammon', `${desc(4)}<q>The demonic embodiment of wealth, Mammon requests a tithe of blood and gold from all his worshippers.</q>`, angelPriceFactor**2, [8, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Belphegor'];
    new Game.Upgrade('Abaddon', `${desc(8)}<q>Master of overindulgence, Abaddon governs the wrinkler brood and inspires their insatiability.</q>`, angelPriceFactor**3, [9, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Mammon'];
    new Game.Upgrade('Satan', `${desc(16)}<q>The counterpoint to everything righteous, this demon represents the nefarious influence of deceit and temptation.</q>`, angelPriceFactor**4, [10, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Abaddon'];
    new Game.Upgrade('Asmodeus', `${desc(32)}<q>This demon with three monstrous heads draws his power from the all-consuming desire for cookies and all things sweet.</q>`, angelPriceFactor**5, [11, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Satan'];
    new Game.Upgrade('Beelzebub', `${desc(64)}<q>The festering incarnation of blight and disease, Beelzebub rules over the vast armies of pastry inferno.</q>`, angelPriceFactor**6, [12, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Asmodeus'];
    new Game.Upgrade('Lucifer', `${desc(128)}<q>Also known as the Lightbringer, this infernal prince's tremendous ego caused him to be cast down from pastry heaven.</q>`, angelPriceFactor**7, [13, 11]); Game.last.pool = 'prestige'; Game.last.parents = ['Beelzebub'];
    ...
    Game.last.choicesFunction = function ()
    {
      ...
      for (let i = 0; i < choices.length; i++) { choices[i].name = loc(choices[i].name); }
    ...
    new Game.Upgrade('Five-finger discount', `${loc("All upgrades are <b>%1% cheaper per %2</b>.",[1,loc("%1 cursor",100)])}<q>Stick it to the man.</q>`, 555555, [28, 7], (()=> {Game.upgradesToRebuild=1;})); Game.last.pool = 'prestige'; Game.last.parents = ['Halo gloves', 'Abaddon'];
    ...
    Game.last.clickFunction = Game.spendLump(1, loc('activate the sugar frenzy'), ()
		=> {
    ...
    {
      Game.last.descFunc = function () {
        let str = `"There once was a baker named ${Game.bakeryName}. One day, there was a knock at the door; ${Game.bakeryName} opened it and was suddenly face-to-face with a strange and menacing old grandma. The grandma opened her mouth and, in a strange little voice, started reciting this strange little tale : `;
        let n = 35;
    ...
    if (EN)
    {
      Game.last.descFunc = function(){
				...
				Math.seedrandom();return this.desc+'<q>Absolutely blasphemous!</q>';
			};
    }
    else Game.last.desc = '<q>-</q>';
    ...
    order = 10040;
    Game.NewUpgradeCookie({ name: 'Profiteroles', desc: 'Also known as cream puffs, these pastries are light, fluffy, filled with whipped cream and fun to throw at people when snowballs are running scarce.', icon: [29, 29], require: 'Box of pastries',		power: 4, price: 10**31 });
    Game.NewUpgradeCookie({ name: 'Jelly donut', desc: 'Guaranteed to contain at least 0.3% jelly filling, or your money back.<br>You can still see the jelly stab wound!', icon: [27, 28], require: 'Box of pastries',		power: 4, price: 10**33 });
    Game.NewUpgradeCookie({ name: 'Glazed donut', desc: 'Absolutely gooey with sugar. The hole is the tastiest part!', icon: [28, 28], require: 'Box of pastries',		power: 4, price: 10**35 });
    Game.NewUpgradeCookie({ name: 'Chocolate cake', desc: 'The cake is a Portal reference!', icon: [25, 27], require: 'Box of pastries',		power: 4, price: 10**37 });
    Game.NewUpgradeCookie({ name: 'Strawberry cake', desc: 'It\'s not easy to come up with flavor text for something as generic as this, but some would say it\'s a piece of cake.', icon: [26, 27], require: 'Box of pastries',		power: 4, price: 10**39 });
    Game.NewUpgradeCookie({ name: 'Apple pie', desc: 'It is said that some grandmas go rogue and bake these instead.', icon: [25, 28], require: 'Box of pastries',		power: 4, price: 10**41 });
    Game.NewUpgradeCookie({ name: 'Lemon meringue pie', desc: 'Meringue is a finicky substance made of sugar and egg whites that requires specific atmospheric conditions to be baked at all. The lemon, as far as we can tell, isn\'t nearly as picky.', icon: [26, 28], require: 'Box of pastries',		power: 4, price: 10**43 });
    Game.NewUpgradeCookie({ name: 'Butter croissant', desc: 'Look around.<br>A rude man in a striped shirt bikes past you. He smells of cigarettes and caf&eacute;-au-lait. Somewhere, a mime uses his moustache to make fun of the British. 300 pigeons fly overhead.<br>Relax. You\'re experiencing croissant.', icon: [29, 28], require: 'Box of pastries',		power: 4, price: 10**45 });

    order = 10050;
    Game.NewUpgradeCookie({ name: 'Cookie dough', desc: 'Bursting with infinite potential, but can also be eaten as is. Arguably worth the salmonella.', icon: [25, 30], require: 'Box of maybe cookies',		power: 4, price: 10**35 });
    Game.NewUpgradeCookie({ name: 'Burnt cookie', desc: 'This cookie flew too close to the sun and is now a shadow of its former self. If only you remembered to set a timer, you wouldn\'t have this tragedy on your hands...', icon: [23, 30], require: 'Box of maybe cookies',		power: 4, price: 10**37 });
    Game.NewUpgradeCookie({ name: 'A chocolate chip cookie but with the chips picked off for some reason', desc: 'This has to be the saddest thing you\'ve ever seen.', icon: [24, 30], require: 'Box of maybe cookies',		power: 3, price: 10**39 });
    Game.NewUpgradeCookie({ name: 'Flavor text cookie', desc: 'What you\'re currently reading is what gives this cookie its inimitable flavor.', icon: [22, 30], require: 'Box of maybe cookies',		power: 4, price: 10**41 });
    Game.NewUpgradeCookie({ name: 'High-definition cookie', desc: 'Uncomfortably detailed, like those weird stories your aunt keeps telling at parties.', icon: [28, 10], require: 'Box of maybe cookies',		power: 5, price: 10**43 });

    order = 10060;
    Game.NewUpgradeCookie({ name: 'Toast', desc: 'A crisp slice of bread, begging for some butter and jam.<br>Why do people keep proposing these at parties?', icon: [27, 10], require: 'Box of not cookies',		power: 4, price: 10**34 });
    Game.NewUpgradeCookie({ name: 'Peanut butter & jelly', desc: 'It\'s time.', icon: [29, 9], require: 'Box of not cookies',		power: 4, price: 10**36 });
    Game.NewUpgradeCookie({ name: 'Wookies', desc: 'These aren\'t the cookies you\'re looking for.', icon: [26, 30], require: 'Box of not cookies',		power: 4, price: 10**38 });
    Game.NewUpgradeCookie({ name: 'Cheeseburger', desc: 'Absolutely no relation to cookies whatsoever - Orteil just wanted an excuse to draw a cheeseburger.', icon: [28, 30], require: 'Box of not cookies',		power: 4, price: 10**40 });
    Game.NewUpgradeCookie({ name: 'One lone chocolate chip', desc: 'The start of something beautiful.', icon: [27, 30], require: 'Box of not cookies',		power: 1, price: 10**42 });
    ...
    let getCookiePrice = function (level) { return 999999999999999999999999999999999999999 * 10**((level-1)/2); };
    ...
    order = 10060;
    Game.NewUpgradeCookie({ name: 'Pizza', desc: 'What is a pizza if not a large, chewy cookie, frosted with a rather exuberant tomato & cheese icing? Not a cookie, that\'s what.', icon: [31, 9], require: 'Box of not cookies',		power: 5, price: 10**44 });

    order = 10050;
    Game.NewUpgradeCookie({ name: 'Crackers', desc: 'These are the non-flavored kind with no salt added. Really just a judgment-free wheat square begging to have bits of ham and spreadable cheese piled onto it, its main contribution being "crunchy".', icon: [30, 9], require: 'Box of maybe cookies',		power: 4, price: 10**45 });
    ...
    order = 10060;
    Game.NewUpgradeCookie({ name: 'Candy', desc: 'There are two pillars to the world of sweets : pastries, of course - and candy.<br>You could make a whole new game just about these, but for now, please enjoy these assorted generic treats.', icon: [30, 10], require: 'Box of not cookies',		power: 5, price: 10**46 });
...
[0, 0]
    ...
    order = 25100;
    let dragonDropUpgradeCost = function (me) { return Game.unbuffedCps * 60 * 30 * ((Game.dragonLevel < Game.dragonLevels.length - 1) ? 1 : 0.1); };
    ...
    Game.NewUnshackleBuilding = function (obj)
    {
      ...
      let upgrade = new Game.Upgrade(`Unshackled ${building.bplural}`, (obj.building == 'Cursor' ? getStrThousandFingersGain(25) : loc('Tiered upgrades for <b>%1</b> provide an extra <b>+%2%</b> production.<br>Only works with unshackled upgrade tiers.', [cap(building.plural), Math.round((building.id == 1 ? 0.5 : (20 - building.id) * 0.1) * 100)])) + (EN ? `<q>${obj.q}</q>` : ''), (building.id+1)**7 * 15000000, [building.iconColumn, 35]);
    ...
    Game.NewUnshackleUpgradeTier = function (obj)
    {
      ...
      let upgrade = new Game.Upgrade(obj.tier == 1 ? 'Unshackled flavor' : `Unshackled ${tier.name.toLowerCase()}`, loc('Unshackles all <b>%1-tier upgrades</b>, making them more powerful.<br>Only applies to unshackled buildings.', cap(loc('[Tier]' + tier.name, 0, tier.name))) + (EN ? `<q>${obj.q}</q>` : ''), obj.tier**7.5 * 10000000, [10, tier.iconRow]);
      upgrade.pool = 'prestige';
    ...
    Game.last.choicesFunction = function ()
    {
      ...
      if (App)
      {
        let data = Music ? Music.tracks[Game.jukebox.tracks[Game.jukebox.onTrack]].audio : 0;
        let dur = data ? data.duration + 1 : 0;
        str += '<div class="line"></div>';
    ...
    order = 10050;
    Game.NewUpgradeCookie({ name: 'Deep-fried cookie dough', desc: 'They\'ll fry anything these days. Drizzled in hot chocolate syrup, just like in state fairs. Spikes up your blood sugar AND your cholesterol!', icon: [23, 35], require: 'Box of maybe cookies', power:	5, price: 10**47 });
    ...
    {
      let num = 0;
      for (let i in Game.santaDrops) { if (Game.Has(Game.santaDrops[i])) num++; }
      return num;
    };
    ...
    {
      let num = 0;
      for (let i in Game.reindeerDrops) { if (Game.Has(Game.reindeerDrops[i])) num++; }
      return num;
    };
    ...
    Game.computeSeasonPrices = function ()
    {
      for (let i in Game.seasons)
      {
        Game.seasons[i].triggerUpgrade.priceFunc = function () {
          ...
          return Game.seasonTriggerBasePrice + Game.unbuffedCps * 60 * 1.5**Game.seasonUses * m;
        };
      }
    };
    ...
    for (var i in Game.Upgrades)
    {
      if (Game.Upgrades[i].pool == 'prestige' || Game.Upgrades[i].pool == 'prestigeDecor')
      {
        ...
        for (let ii in Game.Upgrades[i].parents) { Game.Upgrades[i].parents[ii] = Game.Upgrades[Game.Upgrades[i].parents[ii]]; }
      }
    }
    ...
    Game.Win = function (what)
    {
      ...
      else { for (let i in what) { Game.Win(what[i]); } }
    };
    ...
    Game.CountsAsAchievementOwned = function(pool)
		{
			if (pool=='' || pool=='normal') return true; return false;
		};
    ...
    {
      let achiev = new Game.Achievement(name, loc('Have <b>%1</b>.', loc('%1 ' + Game.Objects[building].bsingle, LBeautify(Game.Tiers[tier].achievUnlock))) + desc, Game.GetIcon(building, tier));
      Game.SetTier(building, tier);
      return achiev;
    };
    ...
    Game.ProductionAchievement = function (name, building, tier, q, mult)
    {
      ...
      let pow = 10**n;
      let achiev = new Game.Achievement(name, loc('Make <b>%1</b> just from %2.', [loc('%1 cookie', { n: pow, b: toFixed(pow) }), building.plural]) + (q ? `<q>${q}</q>` : ''), icon);
      building.productionAchievs.push({ pow, achiev });
      return achiev;
    };
    ...
    {
      let threshold = 10**Math.floor(Game.BankAchievements.length*1.5+2);
      if (Game.BankAchievements.length == 0) threshold = 1;
      let achiev = new Game.Achievement(name, loc('Bake <b>%1</b> in one ascension.', loc('%1 cookie', { n: threshold, b: toFixed(threshold) })) + (q ? (`<q>${q}</q>`) : ''), [Game.thresholdIcons[Game.BankAchievements.length], (Game.BankAchievements.length > 43 ? 2 : Game.BankAchievements.length > 32 ? 1 : Game.BankAchievements.length > 23 ? 2 : 5)]);
    ...
    {
      let threshold = 10**Math.floor(Game.CpsAchievements.length*1.2);
      // if (Game.CpsAchievements.length==0) threshold=1;
      let achiev = new Game.Achievement(name, loc('Bake <b>%1</b> per second.', loc('%1 cookie', { n: threshold, b: toFixed(threshold) })) + (q ? (`<q>${q}</q>`) : ''), [Game.thresholdIcons[Game.CpsAchievements.length], (Game.CpsAchievements.length > 43 ? 2 : Game.CpsAchievements.length > 32 ? 1 : Game.CpsAchievements.length > 23 ? 2 : 5)]);
      achiev.threshold = threshold;
    ...
    Game.hasBuff = function(what)//returns 0 if there is no buff in effect with this name; else, returns it
		{if (!Game.buffs[what]) return 0; return Game.buffs[what];};
    ...
    new Game.buffType('cookie storm', ((time,pow)
		=> {
			return {
				name:'Cookie storm',
    ...
    new Game.buffType('sugar blessing', ((time,pow)
		=> {
			return {
				name:'Sugar blessing',
    ...
    new Game.buffType('loan 1', ((time,pow)
		=> {
			return {
				name:'Loan 1',
				dname:loc("Loan %1",1),
    ...
    new Game.buffType('loan 1 interest', ((time,pow)
		=> {
			return {
    ...
    new Game.buffType('loan 2', ((time,pow)
		=> {
			return {
				name:'Loan 2',
				dname:loc("Loan %1",2),
    ...
    new Game.buffType('loan 2 interest', ((time,pow)
		=> {
			return {
    ...
    new Game.buffType('loan 3', ((time,pow)
		=> {
			return {
				name:'Loan 3',
				dname:loc("Loan %1",3),
    ...
    new Game.buffType('loan 3 interest', ((time,pow)
		=> {
			return {
    ...
    function inRect(x, y, rect)
    {
      ...
      let dx = x + Math.sin(-rect.r) * (-(rect.h / 2 - rect.o)); var  dy = y + Math.cos(-rect.r) * (-(rect.h / 2 - rect.o));
    ...
    Game.UpdateWrinklers = function ()
    {
      ...
      for (var i in Game.wrinklers)
      {
        ...
        if (me.phase > 0)
        {
          ...
          let rect = { w: 100, h: 200, r: (-me.r) * Math.PI / 180, o: 10 };
        ...
        if (me.hp <= 0.5 && me.phase > 0)
        {
          ...
          if (me.sucked > 0.5)
          {
            ...
            if (Game.season == 'halloween')
            {
              ...
              {
                let cookie = choose(['Skull cookies', 'Ghost cookies', 'Bat cookies', 'Slime cookies', 'Pumpkin cookies', 'Eyeball cookies', 'Spider cookies']);
                if (!Game.HasUnlocked(cookie) && !Game.Has(cookie))
                {
                  Game.Unlock(cookie);
    ...
    Game.DrawWrinklers = function ()
    {
      ...
      if (selected && Game.Has('Eye of the wrinkler'))
      {
        ...
        for (var i = 0; i < Math.floor(dist / 12); i++)
        {
          ...
          ctx.beginPath();
          ctx.arc(xC, yC, 4 + (Game.prefs.fancy ? 2 * Math.sin(-Game.T*0.2+i*0.3)**4 : 0), 0, 2 * Math.PI, false);
          ctx.fill();
          ctx.stroke();
        }
    ...
    Game.SaveWrinklers = function ()
    {
      ...
      return { amount, number, shinies, amountShinies };
    };
    ...
    { Game.Upgrades[Game.santaDrops[i]].priceFunc = function () { return 3**Game.santaLevel * 2525; } }

    Game.UpgradeSanta = function ()
    {
      let moni = (Game.santaLevel+1)**(Game.santaLevel+1);
      if (Game.cookies > moni && Game.santaLevel < 14)
      {
        ...
        let drops = [];
        for (let i in Game.santaDrops) { if (!Game.HasUnlocked(Game.santaDrops[i])) drops.push(Game.santaDrops[i]); }
        ...
        if (l('specialPic')) { let rect = l('specialPic').getBounds(); Game.SparkleAt((rect.left + rect.right) / 2, (rect.top + rect.bottom) / 2) + 32 - TopBarOffset; }
    ...
    Game.dragonLevels = [
      { name: 'Dragon egg', action: loc('Chip it'), pic: 0,
        cost(){return Game.cookies>=1000000;},
        buy(){Game.Spend(1000000);},
        costStr(){return loc("%1 cookie",LBeautify(1000000));} },
      { name: 'Dragon egg', action: loc('Chip it'), pic: 1,
        cost(){return Game.cookies>=1000000*2;},
        buy(){Game.Spend(1000000*2);},
        costStr(){return loc("%1 cookie",LBeautify(1000000*2));} },
      { name: 'Dragon egg', action: loc('Chip it'), pic: 2,
        cost(){return Game.cookies>=1000000*4;},
        buy(){Game.Spend(1000000*4);},
        costStr(){return loc("%1 cookie",LBeautify(1000000*4));} },
      { name: 'Shivering dragon egg', action: loc('Hatch it'), pic: 3,
        cost(){return Game.cookies>=1000000*8;},
        buy(){Game.Spend(1000000*8);},
        costStr(){return loc("%1 cookie",LBeautify(1000000*8));} },
      { name: 'Krumblor, cookie hatchling', action: 'Train Breath of Milk<br><small>Aura : kittens are 5% more effective</small>', pic: 4,
        cost(){return Game.cookies>=1000000*16;},
        buy(){Game.Spend(1000000*16);},
        costStr(){return loc("%1 cookie",LBeautify(1000000*16));} },
      ...
      { name: 'Krumblor, cookie dragon', action: `${loc("Bake dragon cookie")}<br><small>${loc("Delicious!")}</small>`, pic: 6,
        cost(){var fail=0;for (var i in Game.Objects){if (Game.Objects[i].amount<50) fail=1;}return (fail==0);},
        buy(){for (var i in Game.Objects){Game.Objects[i].sacrifice(50);}Game.Unlock('Dragon cookie');},
        costStr(){return loc("%1 of every building",50);} },
      { name: 'Krumblor, cookie dragon', action: `${loc("Train secondary aura")}<br><small>${loc("Lets you use two dragon auras simultaneously")}</small>`, pic: 7,
        cost(){var fail=0;for (var i in Game.Objects){if (Game.Objects[i].amount<200) fail=1;}return (fail==0);},
        buy(){for (var i in Game.Objects){Game.Objects[i].sacrifice(200);}},
        costStr(){return loc("%1 of every building",200);} },
    ...
    Game.hasAura = function(what)
		{
			if (Game.dragonAuras[Game.dragonAura].name==what || Game.dragonAuras[Game.dragonAura2].name==what) return true; return false;
		};
    ...
    Game.SelectDragonAura = function (slot, update)
    {
      ...
      Game.Prompt(
`<id PickDragonAura><h3>${loc(slot==1?"Set your dragon's secondary aura":"Set your dragon's aura")}</h3>`+
						`<div class="line"></div>`+
      ...
      , [[loc('Confirm'), `${slot==0?'Game.dragonAura':'Game.dragonAura2'}=Game.SelectingDragonAura;${highestBuilding==0 || currentAura==Game.SelectingDragonAura?'':'Game.ObjectsById['+highestBuilding.id+'].sacrifice(1);'}Game.ToggleSpecialMenu(1);Game.ClosePrompt();`], loc('Cancel')],
0,
'widePrompt'
);
    ...
    Game.ToggleSpecialMenu = function (on)
    {
      if (on)
      {
        ...
        let str = `<div id="specialPic" ${Game.clickStr}="Game.ClickSpecialPic();" style="${(Game.specialTab=='dragon' && Game.dragonLevel>=4 && Game.Has('Pet the dragon'))?'cursor:pointer;':''}position:absolute;left:-16px;top:-64px;width:96px;height:96px;background:url(img/${pic});background-position:${-frame*96}px 0px;filter:drop-shadow(0px 3px 2px #000);-webkit-filter:drop-shadow(0px 3px 2px #000);"></div>`;
    ...
    Game.DrawBackground = function ()
    {
      ...
      else
      {
        ...
        if (Game.AscendTimer == 0)
        {
          if (Game.prefs.particles)
          {
            ...
            if (false)// don't do that
            {
              ...
              let digits = amount.length;
              let space = 0;
              for (var i = 0; i < digits; i++)
              {
                var s = 16 * (digits - i);
                let num = parseInt(amount[i]);
                ...
                for (var ii = 0; ii < num; ii++)
                {
                  var x = Game.cookieOriginX;
                  var y = Game.cookieOriginY;
                  let spin = Game.T * (0.005 + i * 0.001) + i + (ii / num) * Math.PI * 2;
          ...
          if (Game.prefs.cursors)
          {
            ...
            let {fancy} = Game.prefs;

            if (showDragon) ctx.globalAlpha = 0.25;
            var {amount} = Game.Objects['Cursor'];
            // var spe=-1;
            for (var i = 0; i < amount; i++)
            {
        ...
        if (Game.prefs.milk)
        {
          ...
          if (Game.AscendTimer > 0)
          {
            y *= 1 - (Game.AscendTimer/Game.AscendBreakpoint)**2 * 2;
            a *= 1 - (Game.AscendTimer/Game.AscendBreakpoint)**2 * 2;
          }
          else if (Game.ReincarnateTimer > 0)
          {
            y *= 1 - (1-(Game.ReincarnateTimer/Game.ReincarnateDuration))**2 * 2;
            a *= 1 - (1-(Game.ReincarnateTimer/Game.ReincarnateDuration))**2 * 2;
          }
          ...
          if (Game.TOYS)
          {
            ...
            if (!Game.Toy)
            {
              ...
              Game.Toy = function (x, y)
              {
                ...
                let v = Math.random(); let a = 0.5; let b = 0.5;
                if (v <= a) v = b - b * (1-v/a)**3; else v = b + (1 - b) * ((v-a)/(1-a))**3;
            ...
            for (var i in Game.toys)
            {
              ...
              for (var ii in Game.toys)
              {
                ...
                if (it.id != me.id)
                {
                  ...
                  var x2 = it.x + it.xd;
                  var y2 = it.y + it.yd;
                  let dist = Math.sqrt((x1-x2)**2 + (y1-y2)**2) / (me.s / 2 + it.s / 2);
                  if (dist < (Game.toysType == 1 ? 0.95 : 0.75))
                  {
                    let angle = Math.atan2(y1 - y2, x1 - x2);
                    let v1 = Math.sqrt(me.xd**2 + me.yd**2);
                    let v2 = Math.sqrt(it.xd**2 + it.yd**2);
    ...
    {
      Game.popups = 0;
      let leftout = ['Magic shenanigans', 'Occult obstruction', 'Glucose-charged air'];
      for (let i in Game.Upgrades)
      {
    ...
    else if (App.saveData) setTimeout(()=>  { Game.LoadSave(App.saveData); }, 100);
    else setTimeout(()=>  { Game.LoadSave(); }, 100);

    Game.ready = 1;
    setTimeout(()=>  { if (typeof showAds === 'undefined' && (!l('detectAds') || l('detectAds').clientHeight < 1)) Game.addClass('noAds'); }, 500);
    l('offGameMessage').innerHTML = '';
    l('offGameMessageWrap').style.display = 'none';
    Game.Loop();
    Game.Draw();
    ...
    if (!Game.OnAscend && Game.AscendTimer == 0)
    {
      ...
      if (Game.T % (Game.fps * 5) == 0 && Game.ObjectsById.length > 0)// check some achievements and upgrades
      {
        ...
        for (var i in Game.Objects)
        {
          ...
          if (!Game.HasAchiev('Mathematician')) { if (Game.Objects[i].amount < Math.min(128, 2**((Game.ObjectsById.length-Game.Objects[i].id)-1))) mathematician = 0; }
  ...
  Game.Loop = function ()
  {
    ...
    if (Game.sesame)
    {
      ...
      let str = '';
      for (let i in Timer.labels) { str += Timer.labels[i]; }
...
window.onload = function ()
{
  if (!Game.ready)
  {
    ...
    let showLangSelect = function (callback)
    {
      let str = '';
      for (var i in Langs)
      {
